#include <stdint.h>   /* Declarations of uint_32 and the like */
#include <pic32mx.h> 
#include "ili9341.h"
#define FIRST_BLOCK_CORD_X 150
#define FIRST_BLOCK_CORD_Y 40
#define BLOCK_THICC 15
#define BLOCK_LEN 30


const uint8_t const font[] = {
  
};




const uint8_t const icon[] = {
   , 
};

uint32_t level1[36][4] =
    {
        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y, RED},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y, RED},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y, RED},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y, RED},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y, RED},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y, RED},

        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y + BLOCK_THICC + 2, ORANGE},

        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y + 2*BLOCK_THICC + 4, YELLOW},


        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y + 3*BLOCK_THICC + 6, GREEN},

        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y + 4*BLOCK_THICC + 8, BLUE},

        {1, FIRST_BLOCK_CORD_X, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE},
        {1, FIRST_BLOCK_CORD_X - BLOCK_LEN - 2, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE},
        {1, FIRST_BLOCK_CORD_X - 2*BLOCK_LEN - 4, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE},
        {1, FIRST_BLOCK_CORD_X - 3*BLOCK_LEN - 6, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE},
        {1, FIRST_BLOCK_CORD_X - 4*BLOCK_LEN - 8, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE},
        {1, FIRST_BLOCK_CORD_X - 5*BLOCK_LEN - 10, FIRST_BLOCK_CORD_Y + 5*BLOCK_THICC + 10, PURPLE}
};